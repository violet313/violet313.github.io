...
function()
{
    ...
    var _attribs= //lets make a start on abstracting out all the DOM attribute literals
    {
        grid:       "grid-simple",
        toggle:     "toggle",
        cellDetail: "dynamic-cell-detail-simple", //for custom-styling away UI side-efects etc
    };
    ...
    var onRenderIDCell=function(row, cell, value, columnDef, item)
    {
        ...
        else
        {
            ...
            //V313HAX:
            //putting in an extra closing div after the closing toggle div and ommiting a final
            //closing div for the detail ctr div causes the slickgrid renderer to insert our detail div
            //as a new column ;) ~since it wraps whatever we provide in a generic div column container.
            //so our detail becomes a child directly of the row not the cell. nice =)  ~no need
            //to apply a css change to the parent slick-cell to escape the cell overflow clipping.

            //sneaky extra </div> inserted here-----------------v
            html.push("<div class='toggle collapse'></div></div>");

            html.push("<div class='", _attribs.cellDetail, "' ");  //appy custom css to our detail panel
            ...
            html.push("<div>",item._detailContent,"</div>");       //put detail in sub ctr for custom styling purposes
